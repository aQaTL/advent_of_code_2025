	...
	while let Some((mut stream_j, stream_i, particle_count)) = streams.pop_front() {
		...
		match grid.get(stream_j).and_then(|row| row.get(stream_i)) {
			...
			Some(b'^') => {
				...
				let visited_new_stream_1 = visited.contains(&(stream_j, new_stream_1_i));
				let visited_new_stream_2 = visited.contains(&(stream_j, new_stream_2_i));
				...
				if visited_new_stream_1 {
					...
					visited.insert((stream_j, new_stream_1_i));
				}
				...
				if visited_new_stream_2 {
					...
					visited.insert((stream_j, new_stream_2_i));
				}
			}
			None => {
